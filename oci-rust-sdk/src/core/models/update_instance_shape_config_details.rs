...
use super::*;
/// The shape configuration requested for the instance. If provided, the instance will be updated with the resources specified. In the case where some properties are missing, the missing values will be set to the default for the provided {@code shape}. <p> Each shape only supports certain configurable values. If the {@code shape} is provided and the configuration values are invalid for that new {@code shape}, an error will be returned. If no {@code shape} is provided and the configuration values are invalid for the instance's existing shape, an error will be returned.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateInstanceShapeConfigDetails {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub baseline_ocpu_utilization: Option<UpdateInstanceShapeConfigDetailsBaselineOcpuUtilization>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub resource_management: Option<UpdateInstanceShapeConfigDetailsResourceManagement>,
}

impl UpdateInstanceShapeConfigDetails {
    /// Create a new UpdateInstanceShapeConfigDetails
    pub fn new() -> Self {
        Self {
            ocpus: None,
...
impl Default for UpdateInstanceShapeConfigDetails {
    fn default() -> Self {
        Self::new()
    }
}
