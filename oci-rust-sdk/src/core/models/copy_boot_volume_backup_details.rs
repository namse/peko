...
pub struct CopyBootVolumeBackupDetails {
    ...
    /// The OCID of the Vault service key in the destination region which will be the master encryption key for the copied boot volume backup. If you do not specify this attribute the boot volume backup will be encrypted with the Oracle-provided encryption key when it is copied to the destination region. <p> For more information about the Vault service and encryption keys, see [Overview of Vault service](https://docs.oracle.com/iaas/Content/KeyManagement/Concepts/keyoverview.htm) and [Using Keys](https://docs.oracle.com/iaas/Content/KeyManagement/Tasks/usingkeys.htm).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub kms_key_id: Option<String>,
}

/// Required fields for CopyBootVolumeBackupDetails
pub struct CopyBootVolumeBackupDetailsRequired {
    /// The name of the destination region. <p> Example: {@code us-ashburn-1}
    pub destination_region: String,
}

impl CopyBootVolumeBackupDetails {
    /// Create a new CopyBootVolumeBackupDetails with required fields
    pub fn new(required: CopyBootVolumeBackupDetailsRequired) -> Self {
        Self {
