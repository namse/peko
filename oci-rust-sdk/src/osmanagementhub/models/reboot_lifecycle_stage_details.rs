...
pub struct RebootLifecycleStageDetails {
    /// The number of minutes the service waits for the reboot to complete. If the instances in the stage don't reboot within this time, the reboot job status is set to failed. Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
...
impl RebootLifecycleStageDetails {
    /// Create a new RebootLifecycleStageDetails
    pub fn new() -> Self {
        Self {
...
impl Default for RebootLifecycleStageDetails {
    fn default() -> Self {
        Self::new()
    }
}
